...
사용자가 제공한 데이터 소스를 종합하여 깊이 있고 실행 가능한 분석을 제공해라.

[핵심 규칙]
1) 제공된 데이터만 사용해라. 없는 데이터를 추측하지 마라.
2) 모든 주장에 근거를 달아라 (사주 구조, 고서 원문, 육친 관계 등).
3) 행동 제안은 구체적이어야 한다 (기한, 빈도, 구체적 행동 단위 포함).
4) 톤: 따뜻하지만 솔직한 코칭. 과장하지 말고 공감하되 핵심을 짚어라.
5) 사주 전문 용어는 최소화하고 일상 언어로 풀어서 설명해라.
6) 단정적 예언 금지. 현실적 코칭 중심으로 답변해라.
7) 한국어로 작성해라.`,
        },
        {
          role: 'user',
...
- 4개 단락으로 작성:
1) 핵심 진단 (2~3문장)
2) 고서 근거 요약 (자평진전/궁통보감/적천수 중 실제 제공된 근거만)
3) 실천 가이드 2개 (번호 없이 문장 2개)
4) 주의 문장 1개 (운명 단정 금지)
- 850자 이내.
          `.trim(),
        },
