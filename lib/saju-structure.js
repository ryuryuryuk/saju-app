...
export function analyzeSajuStructure(saju) {
  ...
  // 오행 분포 카운트 (천간 4 + 지지 4 = 총 8글자)
  const fiveElements = { '목': 0, '화': 0, '토': 0, '금': 0, '수': 0 };
  Object.values(stemInfo).forEach((s) => {
    if (fiveElements[s.element] !== undefined) fiveElements[s.element]++;
  });
  Object.values(branchInfo).forEach((b) => {
    if (fiveElements[b.element] !== undefined) fiveElements[b.element]++;
  });
